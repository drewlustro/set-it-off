...
def auth_url(app_id, canvas_url, perms=None, display='popup'):
...
#------------------------------------------------------------------------
# Custom Functions
def get_user_data_from_code(code, redirect_uri, client_id, client_secret):
    token = get_access_token_from_code(code, redirect_uri, client_id,
                                       client_secret)
    # TODO deal with a bad token
    if not token:
        raise
    # TODO save the access_token and the expiration
    # TODO move this into a model
    access_token = token['access_token']
    expires = time.time() + int(token['expires'])
    ...
    fields = ','.join(['id',
                       ...
                       'email'])
    try:
        facebook_user = graph.get_object('me', fields=fields)
        # TODO check if the user already exists, if so just log in
    except Exception as e:
        # TODO deal with this exception
        print e
        raise
    facebook_user['access_token'] = access_token
    facebook_user['expires'] = expires
